...
disqueldb.add(`yetkili.${message.author.id}.kadın`, 1)
...
disquel.roles.add(settings.roller.kadınRol)
disquel.roles.remove(settings.roller.kayıtsızrol)
disquel.roles.remove(settings.roller.karantinarol)
message.channel.send(new MessageEmbed()
.setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
.setDescription(`${disquel} Kişisinin ismi \`"${disquelİsim}"\` olarak ayarlandı. Sunucumuzda \`"Kadın"\` Olarak Kayıt Edildi
...
exports.conf = {enabled: true, guildOnly: true, aliases: ["woman", "k"]};
