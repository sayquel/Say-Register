...
member.roles.add(settings.roller.viprol)
message.channel.send(new MessageEmbed().setDescription(`${member} Adlı Kullanıcıya ${settings.roller.viprol} Rolü Verildi`).setColor("#fff619")).then(msg => msg.delete({timeout: 10000}))

message.react(settings.durumlar.dogru)
}  

exports.conf = { enabled: true, guildOnly: true , aliases: ["special"]}
